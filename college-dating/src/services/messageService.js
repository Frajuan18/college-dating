...
export const messageService = {
  ...
  // Send a new message
  // services/messageService.js - Fix the sendMessage function
  ...
  async getConversations(userId) {
    try {
      ...
      const { data: users, error: usersError } = await supabase
        .from("users")
        .select(
          "id, first_name, last_name, full_name, photo_url, university_name",
        )
      ...
      messages.forEach((msg) => {
        ...
        const conversationId = [msg.sender_id, msg.receiver_id]
          .sort()
          .join("_");
        ...
        if (!conversationMap.has(conversationId)) {
          conversationMap.set(conversationId, {
            ...
            lastMessage: msg,
            unreadCount:
              msg.receiver_id === userId && msg.status === "sent" ? 1 : 0,
            messages: [msg],
          });
        } else {
